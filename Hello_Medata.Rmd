...
| `protection`     | *logical* | inside MPA/unfished (`TRUE`) or outside MPA/fished (`FALSE`); linked to `enforcement` level |
...
ggplot(medata) +
  ...
  geom_point(aes(x = lon, y = lat, fill = tmean), colour = "black", pch = 21,
             size = 5, alpha = 0.7) +
  fishualize::scale_fill_fish(option = "Serranus_scriba") +
...
med_meta <- medata %>% 
  distinct(country, site, lon, lat, season, protection, enforcement, total.mpa.ha, size.notake, 
           yr.creation, age.reserve.yr, tmean, sal_mean, pp_mean)
...
pres_abs_mat[16:ncol(pres_abs_mat)] <- ifelse(pres_abs_mat[16:ncol(pres_abs_mat)] > 0, 1, 0) # col 176 is the 1st species
